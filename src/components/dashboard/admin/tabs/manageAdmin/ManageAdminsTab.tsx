...
const ManageAdminsTab: React.FC = () => {
  ...
  const fetchAdmins = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/manage-admin/manage-policy");
      const json = await res.json();
  ...
  const handleRemove = async (id: string) => {
    try {
      const res = await fetch(`/api/manage-admin/${id}/delete-admin`, {
        method: "DELETE",
      });
      const json = await res.json();
      if (res.ok) {
  ...
  const hasPolicy = (policy: string) => currentUser?.admin?.policies?.includes(policy);
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="flex justify-between items-center">
          <div>
            <div className="text-sm font-semibold text-slate-900">Manage Administrators</div>
            <div className="text-xs text-slate-500">Add, edit, and manage administrator access</div>
          </div>

          <div className="flex gap-2">
            {/* Show Delete Requests button for Super Admins */}
            {currentUser?.admin?.role === "superadmin" && (
              <Button variant="outline" size="sm" onClick={() => setShowDeleteReqsModal(true)}>
        ...
        <CardBody className="space-y-3">
          {loading ? <p className="text-sm text-slate-500">Loading...</p> : 
            admins.map((admin) => (
            <div key={admin.id} className="flex justify-between items-center border border-slate-200 rounded-lg p-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-slate-900">{admin.full_name}</span>
                  <span className={`text-[11px] px-2 py-0.5 rounded-full ${admin.role === 'superadmin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={currentUser?.admin?.role === "admin" && admin.role === "superadmin"}
                >
                  Remove
                </Button>
              </div>
      ...
      {/* Modals */}
      {showAddModal && <AddAdminModal onClose={() => setShowAddModal(false)} onRefresh={fetchAdmins} />}
      ...
      {removeTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl w-full max-w-sm p-6">
            <div className="text-lg font-semibold mb-2">Remove Administrator</div>
            ...
            <div className="flex gap-3">
              ...
              <Button variant="danger" className="flex-1" onClick={() => handleRemove(removeTarget.id)}>Confirm</Button>
            </div>
          </div>
        </div>
      )}
