  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await authFetch(
        `/api/manage-admin/manage-policy?id=${admin.id}`,
        {
          method: "PUT",
      ...
      if (!res.ok) {
        throw new Error(`Failed to update policies: ${res.status}`);
      }
      ...
      if (json.success) {
        ...
        toast.error(json.message || "Failed to update policies");
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-2xl">
        <div className="mb-4">
          ...
          <p className="text-sm text-slate-500 font-medium">
            Updating permissions for: {admin.full_name}
          </p>
        </div>
